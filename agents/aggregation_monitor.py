...
*** NGUỒN DỮ LIỆU ***
Cùng quy ước với 2 đặc trưng hành vi trong transaction_classifier.py
(avg_time_between_tx, balance_clustering_flag): đọc state["wallet_tx_history"]
— KHÔNG tự crawl. Field này KHÔNG khai báo trong AMLState (core/state.py),
vì project chưa có module crawl Etherscan thật (xem GHI CHÚ trong
transaction_classifier.py). Nếu key này không có trong state, mọi output
liên quan trả về None/False — KHÔNG bịa dữ liệu để "chạy cho có".
...
def analyze_aggregation(state: AMLState) -> AMLState:
    ...
    aggregated_amount_7d, near_threshold_count_30d, structuring_flag vào state.
    """
    ...
    if not wallet_history:
        # Không có lịch sử — không đủ dữ liệu để đánh giá cộng dồn.
        # KHÔNG gán 0 (0 là giá trị hợp lệ khác với "không biết").
        state["aggregated_amount_7d"] = None
        state["near_threshold_count_30d"] = None
        state["structuring_flag"] = False
        state["thought"] = (
            "AggregationMonitor: không có wallet_tx_history — bỏ qua "
            "kiểm tra structuring cho giao dịch này."
        )
        return state
    ...
    state["structuring_flag"] = structuring_flag
    ...
    state["thought"] = (
        ...
        f"structuring_flag = {structuring_flag}."
    )
...
if __name__ == "__main__":
    ...
    # Kỳ vọng: structuring_flag = True (qua điều kiện (a), không phải (b)
    ...
    print(f"structuring_flag: {updated['structuring_flag']}")   